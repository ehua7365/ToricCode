...
##    plt.show()
    lowest = 20
    x = np.log10(n)
    y = np.log10(totalTime)
    z = np.polyfit(x[lowest:],y[lowest:],1)
    p = np.poly1d(z)
    plt.plot(x,p(x),label='Slope = %f'%p[1])
    plt.errorbar(x,y,fmt='+')
    plt.xlabel(r'Lattice size $\log_{10}(L)$')
    plt.ylabel(r'Computation Time $\log_{10}(t_C)$')
    plt.title(r'log-log plot of computation time $t$ vs. toric lattice size $L$')
    plt.legend(loc='upper left')
